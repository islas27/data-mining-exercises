...
let analyzeDates = function (file, column, splitter) {
  var stream = fs.createReadStream(file)
  var count = 0
  var csvStream = csv(options)
    .on('data', function (data) {
      if (!dateChecker(data[column], splitter)) console.log(data)
      else count++
    })
    .on('end', function () {
      console.log('Done! Valid Dates: ' + count)
    })
  stream.pipe(csvStream)
}

let reorder = function (file, classColumn) {
  let collection = []
  var stream = fs.createReadStream(file)
  var csvStream = csv(options)
    .on('data', function (data) {
...
let unpadDate = function (file, column, splitter, dayPosition, monthPosition, yearPosition) {
  let collection = []
  var stream = fs.createReadStream(file)
  var csvStream = csv(options)
    .on('data', function (data) {
      var date = data[column].split(splitter)
      var newDate = []
      newDate[dayPosition] = parseInt(date[dayPosition - 1], 10)
      newDate[monthPosition] = parseInt(date[monthPosition - 1], 10)
      newDate[yearPosition] = parseInt(date[yearPosition - 1], 10)
      var dateString = newDate[1] + splitter + newDate[2] + splitter + newDate[3]
      data[column] = dateString
      collection.push(data)
    })
    .on('end', function () {
      csv
       .writeToPath('out-ud.csv', collection, options)
       .on('finish', function () {
         console.log('Done!')
       })
    })
...
let switchDates = function (file, columnToFix, dateColumn, place, splitter) {
  let collection = []
  let replacedValues = 0
  var stream = fs.createReadStream(file)
  var csvStream = csv(options)
    .on('data', function (data) {
      var date = data[dateColumn].split(splitter)
      var newValue = parseInt(date[place - 1], 10)
      if (data[columnToFix] !== newValue) {
        data[columnToFix] = newValue
        collection.push(data)
        replacedValues++
      } else collection.push(data)
    })
    .on('end', function () {
      csv
       .writeToPath('out-sd.csv', collection, options)
       .on('finish', function () {
         console.log('Replaced: ' + replacedValues)
         console.log('Done!')
       })
...
let dateChecker = function (data, splitter) {
  var date = data.split(splitter)
  ...
  if (year < 2010 || year > 2016 || month === 0 || month > 12) return false
